  ...
  try{
    if(!API_KEY){
      return res.status(500).json({
        success:false,
        message:"Server Configration failed No key found"
      })
    }
    ...
    try{
      ...
      return res.status(502).json({
        success:false,
        message:"Ai retuen invalid JSON",
        model:usedModel,
        raw:text
      })
    ...
    console.error("AI invoice genration error:",err)
    return res.status(500).json({
      success:false,
      message:"Ai genration failed",
