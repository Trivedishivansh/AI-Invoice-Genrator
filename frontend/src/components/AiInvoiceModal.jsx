...
const AiInvoiceModel = ({open,onClose,onGenerate,initialText = ""}) => {
  ...
  try {
    ...
    console.error("onGenrate handler failed",err)
    const msg = err && (err.message || (typeof err === "string" ? err : JSON.stringify(err)))
    setError(msg || "Failed to genrate.Try again")
      ...
      <div className={aiInvoiceModalStyles.modal}>
        ...
        <div className='mt-4'>
          ...
          <textarea value={text} onChange={(e)=>setText(e.target.value)}
            placeholder={`eg. A person wants a logo design for her organic brand "GreenVibe."Quoted for $120 for 2 logo options and final delivery in PNG and vector format`}rows={8} className={aiInvoiceModalStyles.textarea}></textarea>
